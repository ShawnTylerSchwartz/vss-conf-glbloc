 ...
 */
    ...
    // create global flip array
        ...
        $noconfcount = 0;
        $confcount = 0;
        //constrained to contain equal counts of each type
        while($noconfcount < 4 && $confcount < 4) {
            array_push($flipList, 0);
            $noconfcount++;
            array_push($flipList, 1);
            $confcount++;
        }
        shuffle($flipList);
    ...
    if (!isset($_SESSION['Timestamp'])) {
        ...
        $_SESSION['flipList'] = $flipList; // store randomized conf type list once for all trials
    }
    
    $flipListLock =& $_SESSION['flipList'];
